        ...
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 mt-4 sm:mt-0">
            {questions.length > 0 && !isComplete && (
              <button
                onClick={() => setStudyState({ isComplete: true, isTimerActive: false })}
                className="px-3 sm:px-4 py-1.5 rounded-md text-sm font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 transition-all shrink-0"
              >
                End Session
              </button>
            )}
            {initialTimeLimit !== null && (
              <div className={cn("flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-slate-100 rounded-lg font-mono font-medium shrink-0", timeRemaining && timeRemaining < 30 ? "text-red-500 bg-red-50" : "text-slate-700")}>
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg shrink-0">
              <button
                ...
                className={cn("px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all", language === 'en' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-600 hover:text-slate-900")}
              >
                English
              </button>
              <button
                onClick={() => setStudyState({ language: 'hi' })}
                className={cn("px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all", language === 'hi' ? "bg-white text-indigo-600 shadow-sm" : "text-slate-600 hover:text-slate-900")}
              >
                हिंदी
              </button>
            </div>
